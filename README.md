...
At a glance, reviewers know: which specs govern the changed code, whether any architectural decisions were updated, which gates passed, and which files aren't tracked yet.

But the real moment is when a gate **fails**:
...
> | Metric | Value |
> |--------|-------|
> | Intent Coverage | 🟢 **75%** (6/8 files) |
> | Intent Docs Changed | 0 |
> | Graph Nodes | 42 |
> | Graph Edges | 98 |
> | Gates | 🔴 **3/4 passing — 1 blocking** |
>
> <details><summary>🔴 Gate Failures (blocking merge)</summary>
>
> - ❌ **GATE-0003**: PCI compliance check failed
>   > Payment gateway code changed but `SPEC-0008` (Payment Security Constraints) was not updated.
>   > This gate blocks any PR that modifies `src/services/payment.ts` without a corresponding intent doc update.
>   > **Fix**: Update `intent/payment-gateway/specs/SPEC-0008.md` to reflect the change, then re-run `sigil check`.
>
> </details>
>
> <details><summary>Governed Code Changes</summary>
>
> **Payment Gateway** (`COMP-payment-gateway`)
> - `src/services/payment.ts` ⚠️ gate violation
> - `src/models/charge.ts`
> > Specs: `SPEC-0008` [accepted] | Gates: `GATE-0003` 🔴
>
> </details>
>
> ---
...
A developer touched the payment service. The code looked fine in review. But GATE-0003 knew that any change to `payment.ts` requires updating the PCI compliance spec — and it didn't happen. Sigil caught it. The PR is blocked until the intent is updated.

That's the loop: intent defines constraints, gates enforce them, PRs can't drift.
